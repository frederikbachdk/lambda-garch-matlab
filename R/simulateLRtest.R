...
simulations %>% filter(LR<100) %>% ggplot() + aes(x = LR) + 
  ...
  geom_density(data = chi2, aes(x = chi2), size = 1) +
  geom_vline(xintercept = critval, color = 'tomato', linetype='dashed') +
  annotate("text", x=6.35, y=0.35, angle=90, color='tomato', size=6,
           label="5% critical value = 5.82") +
  labs(x = 'Likelihood Ratio statistic', y = 'Density') +
  theme_classic() +
  theme(text = element_text(size=20)) 
ggsave('LR_distribution.png')
