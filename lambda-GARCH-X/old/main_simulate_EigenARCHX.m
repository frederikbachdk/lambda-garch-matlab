...
omega = [0.5; 0.75];
alpha = [0.1 0.06; 0.05 0.01];   % ARCH term
beta  = [0.85 0; 0 0.77];        % GARCH term
c     = [1/3; 1/3];               % Exogenous term 
gamma = 0.25;                    % Autoregressive coefficient for Z_t 
...
options = optimset('TolFun',1e-8, 'TolX',1e-8, 'MaxFunEvals', 3000000, 'MaxIter', 10000000);
...
for i = 1:M
    ...
    % draw innovations from N_2(0,I_p) and N(0,1)
    ...
    for t = 2:T
        ...
        % simulate O_t
        Z(t,1) = sqrt(1 + gamma*(Z(t-1,1))^2) * v(t,1);

        % simulate X_t
        % i) conditional eigenvalues from lambda-GARCH-X
        Lambda(t,:) = omega + alpha*((V'*X(:,t-1)).^2) + beta*Lambda(t-1,:)' + c*(Z(t-1,1).^2);
    ...
    % estimate
    [thetahat, L_con, exit, output, grad, hess] = fminunc(@(coef) -EigenARCH_loglikelihood(Y,coef(), n), theta0, options);
...
histfit(sqrt(T)*(theta(2,:)-log(omega(1))),100)
title('Figure 1a: $\sqrt{T}(\hat{\omega}_1 - \omega_{1,0})$', 'interpreter', 'latex', 'FontSize', 20)

nexttile
histfit(sqrt(T)*(theta(3,:) - log(omega(2))),100)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [L, sigma2, lambda, persistence] = EigenARCH_loglikelihood(Y, param, n)
...
[L_cont,sigma2, lambda, persistence] = EigenARCH_loglikelihood_cont(Y, param, n);

L = mean(L_cont);
end

function [L, sigma2, lambda, persistence] = EigenARCH_loglikelihood_cont(Y, param, n)
