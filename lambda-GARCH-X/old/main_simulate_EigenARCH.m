...
options = optimset('TolFun',1e-8, 'TolX',1e-10, 'MaxFunEvals',3000000, 'MaxIter', 10000000);
...
for i = 1:M
    ...
    X = zeros(T,p)';        % placeholder for simulated multivariate series
    Lambda = zeros(T,p);    % placeholder for simulated eigenvalue series
    Lambda(1,:) = omega';  % set initial value of Lambda to omega'

    % draw innovations from N_2(0,I_p)
    ...
    for t = 2:T
        ...
        Lambda(t,:) = omega + alpha*((V'*X(:,t-1)).^2) + beta*Lambda(t-1,:)';
    ...
    [thetahat, L_con, exit, output, grad, hess] = fminunc(@(coef) -EigenARCH_loglikelihood(X,coef(), n), theta0, options);
...
writematrix(theta,'theta_sims_lambdagarch.xlsx','Sheet',1,'Range','A1')
%% PLOT SIMULATIONS
...
[~, V, omega, ~, ~, ~, ~, A, B] = EigenARCH_repar(p, n, param); %Fetch (reparameterized) parameter matrices
...
for i = 2:T 
    ...
    lambda(:,i) = omega+A*y(:,i-1).^2 + B*lambda(:,i-1);
